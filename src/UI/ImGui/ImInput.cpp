...
namespace GTS {
	...
	void ImInput::ShowGameCursor(bool a_showHide) {
		SKSE::GetTaskInterface()->AddUITask([a_showHide] {
			RE::UIMessageQueue::GetSingleton()->AddMessage(RE::CursorMenu::MENU_NAME, (a_showHide ? RE::UI_MESSAGE_TYPE::kShow : RE::UI_MESSAGE_TYPE::kHide), nullptr);
		});
	}
	...
	void ImInput::UpdateCursorState() {
		std::lock_guard<std::shared_mutex> lock(m_cursorMutex);

		// Detect if game changed cursor state
		bool actualState = RE::UI::GetSingleton()->IsMenuOpen(RE::CursorMenu::MENU_NAME);
		bool stateChanged = (actualState != m_currentCursorState);

		if (stateChanged) {
			// Update our tracking
			m_currentCursorState = actualState;

			// If we didn't request this change, it was game-initiated
			if (!HasPendingCursorRequest()) {
				m_expectedGameCursorState = actualState;
			}

			// Clear old requests
			ClearOldCursorRequests();
		}

		// Determine if we need cursor visible
		bool desiredState = DetermineShouldShowCursor();

		// Only request change if different from current state
		if (desiredState != m_currentCursorState) {
			RequestCursorStateChange(desiredState, CursorRequestSource::Overlay);
		}
	}

	void ImInput::SetOverlayWantsCursor(bool wantsCursor) {
		{
			std::lock_guard<std::shared_mutex> lock(m_cursorMutex);
			if (m_overlayWantsCursor == wantsCursor) {
				return;
			}
			m_overlayWantsCursor = wantsCursor;
		}

		UpdateCursorState();
	}

	bool ImInput::GetOverlayWantsCursor() const {
		std::shared_lock<std::shared_mutex> lock(m_cursorMutex);
		return m_overlayWantsCursor;
	}

	bool ImInput::IsCursorVisible() const {
		std::shared_lock<std::shared_mutex> lock(m_cursorMutex);
		return m_currentCursorState;
	}

	bool ImInput::ShouldRenderOverlay() const {
		return IsCursorVisible();
	}

	void ImInput::ForceCursorState(bool visible) {
		std::lock_guard<std::shared_mutex> lock(m_cursorMutex);

		while (!m_pendingCursorRequests.empty()) {
			m_pendingCursorRequests.pop();
		}

		ShowGameCursor(visible);
		m_currentCursorState = visible;
		m_expectedGameCursorState = visible;
	}

	ImInput::CursorStateInfo ImInput::GetCursorStateInfo() const {
		std::shared_lock<std::shared_mutex> lock(m_cursorMutex);
		return {
			m_currentCursorState,
			m_overlayWantsCursor,
			m_expectedGameCursorState,
			m_pendingCursorRequests.size()
		};
	}

	bool ImInput::DetectGameCursorChange() {
		bool actualState = RE::UI::GetSingleton()->IsMenuOpen(RE::CursorMenu::MENU_NAME);
		return actualState != m_currentCursorState;
	}

	void ImInput::RequestCursorStateChange(bool newState, CursorRequestSource source) {
		CursorStateRequest request;
		request.wantsVisible = newState;
		request.source = source;
		request.timestamp = GetCurrentTime();
		m_pendingCursorRequests.push(request);

		ShowGameCursor(newState);
		m_currentCursorState = newState;
		m_lastCursorUpdateTime = request.timestamp;
	}

	bool ImInput::DetermineShouldShowCursor() const {
		// Overlay always takes priority
		if (m_overlayWantsCursor) {
			return true;
		}

		// Otherwise respect game's expected state
		return m_expectedGameCursorState;
	}

	void ImInput::ClearOldCursorRequests() {
		uint32_t currentTime = GetCurrentTime();
		while (!m_pendingCursorRequests.empty()) {
			const auto& request = m_pendingCursorRequests.front();
			if (currentTime - request.timestamp > 200) {
				m_pendingCursorRequests.pop();
			}
			else {
				break;
	...
	uint32_t ImInput::GetCurrentTime() const {
		return static_cast<uint32_t>(GetTickCount64());
	}

	bool ImInput::HasPendingCursorRequest() const {
		if (m_pendingCursorRequests.empty()) {
			return false;
		}

		uint32_t currentTime = GetCurrentTime();
		auto request = m_pendingCursorRequests.front();
		return (currentTime - request.timestamp) <= 200;
	}
}
